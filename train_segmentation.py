...
def print_class_mapping():
    print("Class mapping (raw value -> class id -> name):")
    for raw_value, class_id in value_map.items():
        class_name = class_names.get(class_id, "Unknown")
        print(f"  {raw_value:>5} -> {class_id:>2} -> {class_name}")


def scan_unknown_mask_values(masks_dir, max_samples=20):
    """Scan a subset of masks to detect any raw values not in value_map."""
    ...
    mask_files = sorted(mask_files)[:max_samples]
    unknown_values = set()
    for fname in mask_files:
        mask_path = os.path.join(masks_dir, fname)
        mask = Image.open(mask_path)
        raw_values = np.unique(np.array(mask))
        for v in raw_values:
            if int(v) not in value_map:
                unknown_values.add(int(v))

    if unknown_values:
        print("Warning: found unknown raw mask values (not in value_map):")
        print("  " + ", ".join(str(v) for v in sorted(unknown_values)))
    else:
        print("Mask scan: no unknown raw values detected (sampled).")
...
def main():
    ...
    # Preflight checks
    print_class_mapping()
    for split_name, split_dir in [("train", data_dir), ("val", val_dir)]:
        ...
        print(f"{split_name.capitalize()} images: {len(os.listdir(img_dir))}")
        print(f"{split_name.capitalize()} masks:  {len(os.listdir(mask_dir))}")
        scan_unknown_mask_values(mask_dir, max_samples=20)
